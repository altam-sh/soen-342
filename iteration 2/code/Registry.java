...
public class Registry {
	...
	public static void main(String[] args) {
		...
		//all of sensor 1 was created already
		System.out.print("Temperature at " + l1 + " is " + ReadTemperature(l1, map, read) + " degrees Celcius\n\n");
		...
		//now we try to replace a sensor that is faulty
		//lets say we want to replace sensor 1
		System.out.print("\nDEBUGG: let's try to replace a sensor.\n");
		System.out.print("Sensor " + s1.getSensorID() + " will get replace by a new sensor!" + "\n");
		Sensor Snew = new Sensor("99999999", false);
		System.out.print("New sensor ID of the new sensor is: " + Snew.getSensorID() + ".\n");
		ReplaceSensor(s1, Snew, sList, map, read);
		System.out.print("New sensor " + Snew.getSensorID() + " has been successfully replaced! \nLet's try to read the temperature using that sensor!\n");
		System.out.print("Temperature at " + l1 + " is " + ReadTemperature(l1, map, read) + " degrees Celcius\n\n");


		//now for the method readCollection() that returns all the location and its correcpoinding temperature
		System.out.print("DEBUGG: now we will do the reacCollection() method that will\n" +
				"return all the location in the system and its corresponding temperature\n");
		readCollection(map, read);
	...
	public static void ReplaceSensor(Sensor old, Sensor replace, Sensor_list sensors, Map map, Read read) {
		ArrayList<Sensor> sList = sensors.getList();
		ArrayList<Map_item> mList = map.getMap();
		ArrayList<Read_item> rList = read.getRead();


		for (Map_item mapItem : mList) {
			if (mapItem.getS().equals(old)) {
				mapItem.setS(replace);
			}
		}

		for (Read_item readItem : rList) {
			if (readItem.getS().equals(old)) {
				readItem.setS(replace);
			}
		}

		for (Sensor sItem : sList) {
			if (sItem.getSensorID().equals(old.getSensorID())) {
				sItem.setSensorID(replace.getSensorID());
				sItem.setDeployed(replace.getDeployed());
			}
		}
		System.out.print("A sensor has been replaced!\n");
	}

	//An additional functionality would be an operation that will return
	// the entire collection of locations handled by the system together
	// with their corresponding temperatures.

	public static void readCollection(Map map, Read read){
		ArrayList<Map_item> mList = map.getMap();
		ArrayList<Read_item> rList = read.getRead();
		System.out.print("Collection Method -- Returns all location and their temperature.\n");

		for(Map_item mapItem : mList){
			System.out.print("[Location : " + mapItem.getL().getName() + "]-");
			Sensor s = mapItem.getS();
			for (Read_item readItem : rList) {
				if (readItem.getS().getSensorID().equals(s.getSensorID())) {
					System.out.print("[Temperature : " + readItem.getT() + "]\n");
				}

			}

		}
		System.out.print("Collection Completed Successfully!\n");
